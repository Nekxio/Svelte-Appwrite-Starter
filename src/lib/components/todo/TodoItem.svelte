<script lang="ts">
	...
	const toggleEditMode = (): boolean => (isEditing = !isEditing);
...
<div
	class={['card bg-base-100 card-sm h-fit min-h-52 w-full cursor-pointer shadow-sm', isEditing && 'bg-primary/20']}
	...
	<div class="card-body justify-between">
		...
		{:else}
			<div class="flex flex-col gap-4" transition:slide={{ duration: 300 }}>
				<div class={['flex w-full items-start gap-2', todo.isComplete && 'text-gray-500 line-through']}>
					<h2 class="card-title line-clamp-2 break-all">{todo.title}</h2>
					<div class="w-1/6 pt-1">
						<Pencil size={16} class="text-gray-500" />
					</div>
				</div>
				<p class={['line-clamp-4 text-sm break-all text-gray-600', todo.isComplete && 'line-through opacity-70']}>
					{todo?.description}
				</p>
			</div>
		{/if}
