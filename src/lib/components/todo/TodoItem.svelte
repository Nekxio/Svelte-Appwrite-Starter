<script lang="ts">
		...
		superForm(data, {
			...
			onResult: ({ result }) => {
				if (result.type === 'success') {
					if (result?.data?.todo) {
						callback(result.data.todo as R);
					} else if (result?.data?.success) {
						callback(undefined as unknown as R);
					}
				}
			}
		});
	...
	const deleteForm = createForm<TodoDelete, void>({ $id: todo.$id }, todoIdSchema, `delete-${todo.$id}`, () => {
	...
	const { enhance: enhanceStatus, delayed: statusDelayed } = updateStatusForm;
	const { enhance: enhanceDelete, delayed: deleteDelayed } = deleteForm;
	...
	<div class="card-body h-full justify-between">
				...
				{#if $contentMessage}
					<div class="text-red-500">
						<ul class="list-disc pl-5">
							{#each Object.values($contentMessage).flat() as error (error)}
								<li>{error}</li>
							{/each}
						</ul>
					</div>
		...
		{:else}
			<div class="flex flex-col gap-4" transition:slide={{ duration: 300 }}>
				...
				>
					<h2 class="card-title line-clamp-2 text-balance break-words">{todo.title}</h2>
					<div class="w-1/6 pt-1">
						<Pencil size={16} class="text-gray-500" />
					</div>
				</div>
				<p
					class="line-clamp-4 text-sm text-balance break-words text-gray-600"
