<script lang="ts">
	...
	interface Props {
		...
		onComplete?: () => void;
		onTodoCreated?: (todo: Todo, message: string) => void;
	}

	let { form: todoForm, onComplete = () => {}, onTodoCreated = () => {} }: Props = $props();

	const form = superForm(todoForm, {
		validators: zodClient(todoCreateSchema),
		onResult: ({ result }) => {
			if (result.type === 'success' && result?.data) {
				onTodoCreated(result.data.todo, i18n['todo.event_created']());
				onComplete();
			} else {
